...
class NewYorksTimeSupabaseUser extends BaseAuthUser {
  NewYorksTimeSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> newYorksTimeSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = NewYorksTimeSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
